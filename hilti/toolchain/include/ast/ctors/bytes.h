...
public:
    ...
    static auto create(ASTContext* ctx, std::string value, const Meta& meta = {}) {
        return CtorPtr(
            new Bytes({QualifiedType::create(ctx, type::Bytes::create(ctx, meta), true)}, std::move(value), meta));
    }

protected:
    Bytes(Nodes children, std::string value, Meta meta)
        : Ctor(std::move(children), std::move(meta)), _value(std::move(value)) {}

    bool isEqual(const Node& other) const override {
        auto n = other.tryAs<Bytes>();
        if ( ! n )
            return false;
