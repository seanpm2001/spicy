...
public:
    ...
    static auto create(ASTContext* ctx, std::string comment, comment::Separator separator = comment::Separator::Before,
                       Meta meta = {}) {
        return NodeDerivedPtr<Comment>(new Comment({}, std::move(comment), separator, std::move(meta)));
    }

protected:
    Comment(Nodes children, std::string comment, comment::Separator separator, Meta meta)
        : Statement(std::move(children), std::move(meta)), _comment(std::move(comment)), _separator(separator) {}

    bool isEqual(const Node& other) const final {
        auto n = other.tryAs<Comment>();
        if ( ! n )
            return false;

        return Statement::isEqual(other) && _comment == n->_comment && _separator == n->_separator;
    }
