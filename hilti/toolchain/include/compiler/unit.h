...
public:
    ...
    /** Returns the unique ID of the unit's module. */
     ...
     */
    std::vector<module::UID> dependencies(bool recursive = false) const;
...
private:
    ...
    Unit(const std::shared_ptr<Context>& context, module::UID uid) : _context(context), _uid(std::move(uid)) {}
    Unit(const std::shared_ptr<Context>& context, module::UID uid, const detail::cxx::Unit& cxx_unit)
        : _context(context), _uid(std::move(uid)), _cxx_unit(std::move(cxx_unit)) {}

    Result<detail::cxx::Unit> _codegenModule(const module::UID& uid);

    std::weak_ptr<Context> _context;            // global context
    module::UID _uid;                           // module's globally unique ID
