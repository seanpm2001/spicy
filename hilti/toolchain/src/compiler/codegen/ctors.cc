...
struct Visitor : hilti::visitor::PreOrder {
    ...
    void operator()(ctor::Library* n) final {
        result = fmt("%s(%s)", n->type()->as<type::Library>()->cxxName(), cg->compile(n->value()));
    }
    ...
    void operator()(ctor::Map* n) final {
        if ( n->valueType()->isA<type::Unknown>() ) {
    ...
    void operator()(ctor::String* n) final { result = fmt("std::string(\"%s\")", util::escapeUTF8(n->value(), true)); }
