...
struct Visitor : hilti::visitor::PreOrder {
    ...
    void operator()(expression::Assign* n) final {
        result = {fmt("%s = %s", cg->compile(n->target(), true), cg->compile(n->source())), cxx::Side::LHS};
    }
    ...
    void operator()(expression::Keyword* n) final {
        switch ( n->kind() ) {
            case expression::keyword::Kind::Self: result = {cg->self(), cxx::Side::LHS}; break;
            case expression::keyword::Kind::DollarDollar: result = {cg->dollardollar(), cxx::Side::LHS}; break;
            case expression::keyword::Kind::Captures: result = {"__captures", cxx::Side::LHS}; break;
            case expression::keyword::Kind::Scope: {
                ...
                result = {fmt("std::string(%s)", scope), cxx::Side::RHS};
                break;
            }
    ...
    void operator()(const expression::Name* n) {
        ...
        if ( auto p = n->declaration()->tryAs<declaration::Parameter>(); p && p->isTypeParameter() ) {
            if ( type::isReferenceType(p->type()) )
                ...
                result = {fmt("%s->__p_%s", cg->self(), p->id()), cxx::Side::LHS};
