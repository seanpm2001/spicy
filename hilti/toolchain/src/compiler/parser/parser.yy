...
static hilti::UnqualifiedTypePtr iteratorForType(hilti::Builder* builder, hilti::QualifiedTypePtr t, hilti::Meta m) {
    if ( hilti::type::isIterable(t) )
        return t->type()->iteratorType();
    else {
        hilti::logger().error(hilti::util::fmt("type '%s' is not iterable", t), m.location());
        return builder->typeError(m);
        }
}

static hilti::UnqualifiedTypePtr viewForType(hilti::Builder* builder, hilti::QualifiedTypePtr t, hilti::Meta m) {
...
%type <hilti::UnqualifiedTypePtr>               base_type_no_attrs base_type type function_type tuple_type struct_type enum_type union_type
...
constant_decl : opt_linkage CONST scoped_id '=' expr ';'
              ...
              | opt_linkage CONST type scoped_id '=' expr ';'
                                                 { $$ = builder->declarationConstant($4, builder->qualifiedType($3, true), $6, $1, __loc__); }
              ;
...
func_param      : opt_func_param_kind qtype local_id opt_func_default_expr opt_attributes
...
try_catch     : CATCH '(' qtype local_id ')' block
                                                 { $$ = builder->statementTryCatch(builder->declarationParameter($4, $3, declaration::parameter::Kind::In, {}, {}, __loc__), std::move($6), __loc__); }
...
base_type_no_attrs
              ...
              | ITERATOR type_param_begin qtype type_param_end           { $$ = iteratorForType(builder, std::move($3), __loc__); }
              ...
              | EXCEPTION ':' type               { $$ = builder->typeException(std::move($3), __loc__); }
...
qtype         : type                             { $$ = builder->qualifiedType(std::move($1), false, __loc__); }
              | CONST type                       { $$ = builder->qualifiedType(std::move($2), true, __loc__); }
...
expr_e        : BEGIN_ '(' expr ')'              { $$ = builder->expressionUnresolvedOperator(hilti::operator_::Kind::Begin, {std::move($3)}, __loc__); }
              ...
              | TYPEINFO '(' base_type ')'       { $$ = builder->expressionTypeInfo(builder->expressionType(builder->qualifiedType(std::move($3), false)), __loc__); }
              | expr_f                           { $$ = std::move($1); }
...
ctor          : CBOOL                            { $$ = builder->ctorBool($1, __loc__); }
              ...
              | CSTRING                          { $$ = builder->ctorString($1, __loc__); }
...
opt_attributes
              : opt_attributes attribute        { $1->add($2); $$ = $1; }
