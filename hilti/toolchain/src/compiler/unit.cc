...
Result<std::shared_ptr<Unit>> Unit::fromSource(const std::shared_ptr<Context>& context,
    ...
    auto uid = context->astContext()->parseSource(path);
    if ( ! uid )
        return uid.error();
...
Result<std::shared_ptr<Unit>> Unit::fromCXX(const std::shared_ptr<Context>& context, const detail::cxx::Unit& cxx,
                                            const hilti::rt::filesystem::path& path) {
    auto uid = module::UID({}, path.native());
    return std::shared_ptr<Unit>(new Unit(context, uid, cxx));
}

Unit::~Unit() {}

ModulePtr Unit::module() const { return context()->astContext()->getModule(_uid); }

bool Unit::isCompiledHILTI() const { return _uid.id && context()->astContext()->getModule(_uid) != nullptr; }

Result<Nothing> Unit::print(std::ostream& out) const {
    if ( auto m = module() )
        detail::printAST(module(), out);
...
Result<detail::cxx::Unit> Unit::_codegenModule(const module::UID& uid) {
    auto module = context()->astContext()->getModule(_uid);
    assert(module);

    HILTI_DEBUG(logging::debug::Compiler, fmt("compiling module %s to C++", _uid));
    logging::DebugPushIndent _(logging::debug::Compiler);

    auto cxx = detail::CodeGen(context()).compileModule(module, this, true);
    ...
    if ( ! cxx )
        logger().internalError(
            fmt("code generation for module %s failed, but did not log error (%s)", _uid, cxx.error().description()));
...
std::vector<module::UID> Unit::dependencies(bool recursive) const {
